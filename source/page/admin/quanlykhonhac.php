							...
							<div class="card">
								<div class="card-header">
									<div class="d-flex align-items-center">
										<h4 class="card-title">Quản lý kho nhạc</h4>
											<button class="btn btn-primary btn-round ml-auto">
											<a href="../uploadmusic/" style="color: white;">
...
<?php
  $html = '';
  $sql = 'SELECT music.id, music.image, music.name, singer.name as singer, music.src, music.duration,music.country, music.date_add 
  FROM music, singer, musicsinger 
  WHERE music.id = musicsinger.music 
  ...
  if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
		...
		$html .= '<td>'.$row["id"].'</td>';
		$html .= '<td>'.$row["name"].'</td>';
		$html .= '<td>'.$row["singer"].'</td>';
		$sql1 = "SELECT * FROM country where id =". $row["country"];
		...
		$html .= <<<EOF
					...
					EOF;
		$html .= '<a href="./delete_music.php?id='.$row["id"].'" onclick="return confirm("Bạn chắc chắn muốn xóa?")"><i class="fa fa-times"></i></a>';
		$html .= '</button>';
		$html .= '</div>';
		$html .= '</td>';
        $html .= '</tr>';
